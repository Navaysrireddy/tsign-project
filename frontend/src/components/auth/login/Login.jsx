...
const Login = () => {
  ...
  // Validate all roles (student, college, recruiter)
  const validate = () => {
    const newErrors = {};
    const { emailOrMobile, password } = form;

    if (!role) newErrors.role = 'Select a login type.';

    if (!emailOrMobile)
      newErrors.emailOrMobile = 'Enter your email or mobile number.';
    else if (mobileRegex.test(emailOrMobile)) {
      if (role !== 'student')
        newErrors.emailOrMobile = 'Only students can login using mobile number.';
    } else if (emailRegex.test(emailOrMobile)) {
      if (role === 'college' && !emailOrMobile.includes('college'))
        newErrors.emailOrMobile = 'College email must include “college”.';
      if (role === 'recruiter' && !emailOrMobile.includes('recruiter'))
        newErrors.emailOrMobile = 'Recruiter email must include “recruiter”.';
    } else newErrors.emailOrMobile = 'Invalid email or mobile format.';

    if (!mobileRegex.test(emailOrMobile) && !otpSent) {
      if (!password) newErrors.password = 'Password required.';
      else if (password.length < 6)
        newErrors.password = 'Minimum 6 characters required.';
    }

    if (!rememberMe) newErrors.rememberMe = 'You must check Remember me.';

    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };

  // Send OTP for student mobile login
  const handleSendOtp = async () => {
    if (!mobileRegex.test(form.emailOrMobile)) {
      setErrors({ emailOrMobile: 'Enter valid 10-digit mobile number for OTP.' });
      return;
    }
    try {
      await axios.post('http://localhost:5000/api/send-otp', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (otpSent && role === 'student') {
        const res = await axios.post('http://localhost:5000/api/verify-otp', {
          mobile: form.emailOrMobile,
          otp: form.otp,
        });
        if (res.data.success) navigate('/studentdashboard');
      ...
      const dummyUsers = [
        ...
        { email: 'college@example.com', password: 'college123', role: 'college' },
        { email: 'recruiter@example.com', password: 'recruiter123', role: 'recruiter' },
      ];

      const user = dummyUsers.find(
        (u) =>
          u.email === form.emailOrMobile &&
  ...
  // Render Conditional Input Fields
  const renderFields = () => {
    if (!role) return null;

    if (role === 'student') {
      const isMobile = mobileRegex.test(form.emailOrMobile);
      return (
        <>
          <div>
            ...
            {errors.emailOrMobile && (
              <p className="text-xs text-red-600">{errors.emailOrMobile}</p>
            )}
          </div>
    ...
    // College and Recruiter
    const placeholders = {
      college: 'Enter college email (must include college)',
      recruiter: 'Enter recruiter email (must include recruiter)',
    };
  ...
  return (
    <div
      ...
      className="min-h-screen flex items-center justify-center bg-cover bg-center bg-fixed px-4"
    >
      <div className="bg-white/90 p-8 rounded-xl max-w-md w-full shadow-lg relative">
        <img src={TsignLogo} alt="logo" className="mx-auto mb-6 w-48" />
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <div className="flex justify-between mt-2 gap-2">
              {['student', 'college', 'recruiter'].map((r) => (
                <label
                  key={r}
                  onClick={() => setRole(r)}
          ...
          {errors.rememberMe && (
            <p className="text-xs text-red-600">{errors.rememberMe}</p>
          )}

          <button
            type="submit"
            disabled={loading}
            className={`w-full py-3 rounded-md text-white mt-2 ${
              loading ? 'bg-blue-400' : 'bg-blue-600 hover:bg-blue-700'
            }`}
          >
            {loading ? 'Loading...' : `Login as ${role || 'User'}`}
          </button>
