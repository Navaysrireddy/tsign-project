...
const handleCollegeInputChange = (e) => {
  ...
  if (name === 'yearOfEstablishment') {
    // Limit value length to 7 (YYYY-MM)
    const truncatedValue = value.slice(0, 7);
    if (truncatedValue === '' || validateMonthYear(truncatedValue)) {
      setCollegeFormData(prev => ({ ...prev, [name]: truncatedValue }));
    }
  } else if (['phone', 'directorContact', 'poCMobile'].includes(name)) {
    // Remove all non-numeric chars from input value
    let numericValue = value.replace(/[^0-9]/g, '');
    // Limit length to max 10 digits
...
// eslint-disable-next-line
  ...
  if (['phone', 'mobileno'].includes(name)) {
    // Remove any non-digit characters
    let numericValue = value.replace(/[^0-9]/g, '');
    // Optionally, limit length to 10 digits max
    if (numericValue.length > 10) {
      numericValue = numericValue.slice(0, 10);
    }
  ...
  } else if (['recruiterName', 'designation'].includes(name)) {
    // Allow only alphabets and space
    const alphaValue = value.replace(/[^a-zA-Z\s]/g, '');
    setRecruiterFormData(prev => ({
      ...prev,
...
const validatePassword = (password) => {
 ...
 const handleCollegeSubmit = (e) => {
  ...
  // Validate password strength internally but no alert for failure
  if (!validatePassword(collegeFormData.pAssword)) {
    // Just prevent submit and do nothing else (no alert)
    return;
  }

  // Confirm password match check internally without alert
...
const handleRecruiterSubmit = (e) => {
  ...
  {collegeFormData.pAssword && !validatePassword(collegeFormData.pAssword) && (
...
<div className="mb-7">
  ...
  {collegeFormData.confirmpassword && collegeFormData.confirmpassword !== collegeFormData.pAssword && (
    <p className="text-red-600 text-sm mt-1">
  ...
  {recruiterFormData.password && !validatePassword(recruiterFormData.password) && (
    <p className="text-red-600 text-sm mt-1">
      Password must be at least 8 characters, include uppercase, lowercase, number, and special character.
    </p>
  )}
...
<div className="mb-7">
  ...
  {recruiterFormData.confirmPassword && recruiterFormData.confirmPassword !== recruiterFormData.password && (
    <p className="text-red-600 text-sm mt-1">
      Passwords do not match.
    </p>
  )}
