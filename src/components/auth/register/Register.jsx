...
//   // Handlers for role, forms, inputs, streams, courses, and files
...
//       const index = currentStreams.indexOf(streamId);
//       if (index > -1) {
//         currentStreams.splice(index, 1);
//       } else {
//         currentStreams.push(streamId);
//       }
//       // Reset courses to those belonging to selected streams
//       const updatedCourses = prev.courses.filter(courseId =>
//         currentStreams.some(stream =>
//           availableCourses[stream]?.some(course => course.id === courseId)
//         )
//       );
//       return { ...prev, streams: currentStreams, courses: updatedCourses };
//     });
//   };
...
//     const { name, value } = e.target;
//     setRecruiterFormData(prev => ({ ...prev, [name]: value }));
//   };
...
//     e.preventDefault();
//     if (collegeFormData.pAssword !== collegeFormData.confirmpassword) {
//       alert('College password and confirm password do not match!');
//       return;
//     }
...
//     e.preventDefault();
//     if (recruiterFormData.password !== recruiterFormData.confirmPassword) {
//       alert('Recruiter password and confirm password do not match!');
//       return;
//     }
...
//         <h2 className="text-3xl font-bold bg-gradient-to-r from-blue-900 to-blue-600 bg-clip-text text-transparent text-center relative pb-4">
...
//               activeRole === 'student'
//                 ? 'bg-gradient-to-br from-blue-900 to-blue-700 text-white border-transparent shadow-lg scale-105'
//                 : ''
...
//         {/* College multi-step form */}
//         {activeRole === 'college' && (
//           <div className="multi-step-form">
//             {/* Steps and form based on collegeStep */}
//             {collegeStep === 1 && (
...
//                     <input type="text" name="collegeName" value={collegeFormData.collegeName} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Registration No*</label>
//                     <input type="text" name="registrationNo" value={collegeFormData.registrationNo} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                 </div>
//                 <div className="flex gap-8 mb-7">
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Email ID*</label>
//                     <input type="email" name="email" value={collegeFormData.email} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Contact No*</label>
//                     <input type="tel" name="phone" value={collegeFormData.phone} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                 </div>
//                 <div className="mb-7">
//                   <label className="block text-gray-700 font-medium mb-2">Address*</label>
//                   <input type="text" name="address" value={collegeFormData.address} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
//                 <div className="mb-7">
//                   <label className="block text-gray-700 font-medium mb-2">State*</label>
//                   <input type="text" name="state" value={collegeFormData.state} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
//                 <div className="flex gap-8 mb-7">
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Create Password*</label>
//                     <input type="password" name="pAssword" value={collegeFormData.pAssword} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Confirm Password*</label>
//                     <input type="password" name="confirmpassword" value={collegeFormData.confirmpassword} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                 </div>
//                 <div className="flex gap-8 mb-7">
//                   <div className="flex-1">
...
//                       <input type="file" id="college-logo" name="logo" onChange={(e) => handleFileChange(e, setCollegeFormData)} accept="image/*" className="absolute left-0 top-0 opacity-0 w-full h-full cursor-pointer" />
//                       <label htmlFor="college-logo" className="block p-4 bg-blue-50 border-2 border-dashed border-blue-200 rounded-lg text-center cursor-pointer hover:bg-blue-100 hover:border-blue-400 transition-colors">Choose File</label>
//                     </div>
...
//                     <div className="relative overflow-hidden">
//                       <input type="file" id="college-certificate" name="institutionCertificate" onChange={(e) => handleFileChange(e, setCollegeFormData)} accept="image/*" className="absolute left-0 top-0 opacity-0 w-full h-full cursor-pointer" />
//                       <label htmlFor="college-certificate" className="block p-4 bg-blue-50 border-2 border-dashed border-blue-200 rounded-lg text-center cursor-pointer hover:bg-blue-100 hover:border-blue-400 transition-colors">Choose File</label>
//                     </div>
...
//                   <select name="institutionSector" value={collegeFormData.institutionSector} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors">
...
//                   <input type="month" name="yearOfEstablishment" value={collegeFormData.yearOfEstablishment} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
...
//                         <input type="checkbox" id={`stream-${stream.id}`} checked={collegeFormData.streams.includes(stream.id)} onChange={() => handleStreamChange(stream.id)} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" />
//                         <label htmlFor={`stream-${stream.id}`} className="ml-2 block text-sm text-gray-700">{stream.name}</label>
//                       </div>
//                     ))}
//                   </div>
//                 </div>
...
//                             <input type="checkbox" id={`course-${course.id}`} checked={collegeFormData.courses.includes(course.id)} onChange={() => handleCourseChange(course.id)} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" />
//                             <label htmlFor={`course-${course.id}`} className="ml-2 block text-sm text-gray-700">{course.name}</label>
//                           </div>
//                         ))
//                       )
//                     ) : (
...
//                   <input type="text" name="directorName" value={collegeFormData.directorName} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
//                 <div className="flex gap-8 mb-7">
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Director Email*</label>
//                     <input type="email" name="directorEmail" value={collegeFormData.directorEmail} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Director Mobile*</label>
//                     <input type="tel" name="directorContact" value={collegeFormData.directorContact} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                 </div>
...
//                   <input type="text" name="poCName" value={collegeFormData.poCName} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
//                 <div className="flex gap-8 mb-7">
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">POC Email*</label>
//                     <input type="email" name="poCEmail" value={collegeFormData.poCEmail} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">POC Mobile*</label>
//                     <input type="tel" name="poCMobile" value={collegeFormData.poCMobile} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                 </div>
//                 <div className="mb-7">
//                   <label className="block text-gray-700 font-medium mb-2">Designation*</label>
//                   <input type="text" name="designation" value={collegeFormData.designation} onChange={handleCollegeInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
...
//                     Back
//                   </button>
//                   <button type="submit" disabled={collegeFormData.streams.length === 0 || collegeFormData.courses.length === 0} className="bg-gradient-to-r from-blue-700 to-blue-900 text-white py-3 px-8 rounded-lg font-semibold uppercase tracking-wide shadow-md hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed">
//                     Next
//                   </button>
//                 </div>
//               </form>
...
//                     <p className="flex"><span className="font-semibold text-blue-900 min-w-48">Streams:</span> {collegeFormData.streams.length > 0 ? collegeFormData.streams.map(getStreamNameById).join(', ') : 'None selected'}</p>
//                     <p className="flex"><span className="font-semibold text-blue-900 min-w-48">Courses:</span> {collegeFormData.courses.length > 0 ? collegeFormData.courses.map(getCourseNameById).join(', ') : 'None selected'}</p>
...
//                 <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-all duration-300 ${recruiterStep === 1 ? 'bg-gradient-to-br from-blue-700 to-blue-900 text-white border-2 border-blue-700 scale-110' : 'bg-white border-2 border-blue-100 text-gray-400'}`}>1</div>
...
//                 <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-all duration-300 ${recruiterStep === 2 ? 'bg-gradient-to-br from-blue-700 to-blue-900 text-white border-2 border-blue-700 scale-110' : 'bg-white border-2 border-blue-100 text-gray-400'}`}>2</div>
...
//                 <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-all duration-300 ${recruiterStep === 3 ? 'bg-gradient-to-br from-blue-700 to-blue-900 text-white border-2 border-blue-700 scale-110' : 'bg-white border-2 border-blue-100 text-gray-400'}`}>3</div>
...
//                   <input type="text" name="companyName" value={recruiterFormData.companyName} onChange={handleRecruiterInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
//                 <div className="flex gap-8 mb-7">
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Website</label>
//                     <input type="url" name="website" value={recruiterFormData.website} onChange={handleRecruiterInputChange} className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Email ID*</label>
//                     <input type="email" name="email" value={recruiterFormData.email} onChange={handleRecruiterInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                 </div>
//                 <div className="flex gap-8 mb-7">
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Contact No*</label>
//                     <input type="tel" name="phone" value={recruiterFormData.phone} onChange={handleRecruiterInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Company Category*</label>
//                     <select name="category" value={recruiterFormData.category} onChange={handleRecruiterInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors">
...
//                 <div className="mb-7">
//                   <label className="block text-gray-700 font-medium mb-2">Address*</label>
//                   <input type="text" name="address" value={recruiterFormData.address} onChange={handleRecruiterInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
//                 <div className="flex justify-end mt-12">
//                   <button type="submit" className="bg-gradient-to-r from-blue-700 to-blue-900 text-white py-3 px-8 rounded-lg font-semibold uppercase tracking-wide shadow-md hover:shadow-lg transition-all">
//                     Next
...
//                   <input type="text" name="recruiterName" value={recruiterFormData.recruiterName} onChange={handleRecruiterInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
//                 <div className="flex gap-8 mb-7">
//                   <div className="flex-1">
//                     <label className="block text-gray-700 font-medium mb-2">Designation*</label>
//                     <input type="text" name="designation" value={recruiterFormData.designation} onChange={handleRecruiterInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                   </div>
//                   <div className="flex-1">
...
//                   <div className="relative overflow-hidden">
//                     <input type="file" id="company-logo" name="logo" onChange={(e) => handleFileChange(e, setRecruiterFormData)} accept="image/*" className="absolute left-0 top-0 opacity-0 w-full h-full cursor-pointer" />
...
//                   <button
//                     type="button"
//                     onClick={prevRecruiterStep}
//                     className="bg-gray-600 text-white py-3 px-8 rounded-lg font-semibold uppercase tracking-wide shadow-md hover:bg-gray-700 hover:shadow-lg transition-all"
//                   >
//                     Back
//                   </button>
//                   <button
//                     type="submit"
//                     className="bg-gradient-to-r from-blue-700 to-blue-900 text-white py-3 px-8 rounded-lg font-semibold uppercase tracking-wide shadow-md hover:shadow-lg transition-all"
//                   >
//                     Next
//                   </button>
//                 </div>
...
//                 <div className="mb-7">
//                   <label className="block text-gray-700 font-medium mb-2">Create Password*</label>
//                   <input type="password" name="password" value={recruiterFormData.password} onChange={handleRecruiterInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
//                 <div className="mb-7">
//                   <label className="block text-gray-700 font-medium mb-2">Confirm Password*</label>
//                   <input type="password" name="confirmPassword" value={recruiterFormData.confirmPassword} onChange={handleRecruiterInputChange} required className="w-full p-3 border border-blue-100 rounded-lg bg-blue-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-colors" />
//                 </div>
//                 <div className="bg-blue-50 p-8 rounded-lg border-l-4 border-blue-600 mb-8">
//                   <h3 className="text-xl font-semibold text-blue-900 mb-6 pb-3 border-b border-blue-200 flex items-center">
//                     <span className="w-6 h-6 bg-green-500 text-white rounded-full flex items-center justify-center text-sm mr-3">✓</span>
...
//                   <button
//                     type="submit"
//                     className="bg-gradient-to-r from-blue-700 to-blue-900 text-white py-3 px-8 rounded-lg font-semibold uppercase tracking-wide shadow-md hover:shadow-lg transition-all animate-pulse"
//                   >
//                     Submit
//                   </button>
//                 </div>
...
const Register = () => {
 ...
 const handleCollegeInputChange = (e) => {
  ...
  if (name === 'yearOfEstablishment') {
   ...
   if (limitedValue === '' || validateMonthYear(limitedValue)) {
      setCollegeFormData(prev => ({ ...prev, [name]: limitedValue }));
     } 
  
    
  } else if (['phone', 'directorContact', 'poCMobile'].includes(name)) {
    // Remove all non-numeric chars from input value before updating state
    const numericValue = value.replace(/[^0-9]/g, '');
    setCollegeFormData(prev => ({ ...prev, [name]: numericValue }));
  } else if (['collegeName','state', 'directorName', 'pocName'].includes(name)) {
    // Remove digits and special chars, allow letters and spaces only
    const alphaValue = value.replace(/[^a-zA-Z\s]/g, '');
    setCollegeFormData(prev => ({ ...prev, [name]: alphaValue }));
  } else {
    setCollegeFormData(prev => ({ ...prev, [name]: value }));
  }
};
...
// eslint-disable-next-line
  ...
  e.preventDefault();
  console.log('Recruiter form submitted:', recruiterFormData);
  // Save data so it can be shown later in profile
  localStorage.setItem('recruiterData', JSON.stringify(recruiterFormData));
  alert('Recruiter registration successful!');
  navigate('/recruiter/dashboard'); // or wherever recruiter dashboard is
};
