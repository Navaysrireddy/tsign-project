...
import { VideoIcon, CalendarIcon, ClipboardCheckIcon, AlertCircleIcon } from 'lucide-react';
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full">
        <thead
          className={`${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-50'} border-b ${
            isDarkMode ? 'border-gray-700' : 'border-gray-200'
          }`}
        >
          <tr>
            <th className={`p-4 text-left text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Candidate
            </th>
            <th className={`p-4 text-left text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Position
