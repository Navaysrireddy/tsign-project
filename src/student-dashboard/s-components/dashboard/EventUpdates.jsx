  ...
  const events = [
    {
      ...
      type: 'company',
      urgent: true,
      link: 'https://example.com/company-visits' // 🔗 related website
    },
    {
      id: 2,
      title: 'Workshop by Company X',
      ...
      {events.some(event => event.urgent) && (
        <div className="mb-4 overflow-hidden bg-red-50 dark:bg-red-900/20 rounded-lg p-2">
          <div className="flex whitespace-nowrap animate-marquee">
            {events.filter(event => event.urgent).map(event => (
              <div
                key={`marquee-${event.id}`}
                className="mr-8 text-red-600 dark:text-red-400 font-medium flex items-center"
              >
                <span className="mr-2">🔴</span>
                {event.title} - {event.date}
              </div>
        ...
        {events.map(event => (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <span
                className={`text-xs px-2 py-1 rounded-full ${getTypeColor(event.type)}`}
              >
                {event.type.charAt(0).toUpperCase() + event.type.slice(1)}
              </span>
            </div>
