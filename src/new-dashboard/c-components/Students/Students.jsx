...
const students = [
  {
    ...
    year: 4,
    gpa: 8.5,
    placement: { status: 'Placed', company: 'Google', package: '12 LPA' }
  },
  {
    id: 'stu2',
    name: 'Jane Smith',
    ...
    year: 3,
    gpa: 7.8,
    placement: { status: 'Not Placed', company: '', package: '' }
  },
  {
    id: 'stu3',
    name: 'Alice Johnson',
    ...
    year: 2,
    gpa: 9.2,
    placement: { status: 'Placed', company: 'Microsoft', package: '15 LPA' }
  },
  {
    id: 'stu4',
    name: 'Bob Williams',
    ...
    year: 2,
    gpa: 8.1,
    placement: { status: 'Placed', company: 'Amazon', package: '14 LPA' }
  },
  {
    id: 'stu5',
    name: 'Charlie Brown',
  ...
  },
  {
    ...
    year: 4,
    gpa: 8.9,
    placement: { status: 'Placed', company: 'Tesla', package: '18 LPA' }
  },
  {
    id: 'stu7',
    name: 'Eva Davis',
  ...
  },
  {
    ...
    year: 3,
    gpa: 8.2,
    placement: { status: 'Placed', company: 'Infosys', package: '10 LPA' }
  },
  {
    id: 'stu9',
    name: 'Grace Lee',
  ...
  },
  {
    ...
    year: 2,
    gpa: 8.0,
    placement: { status: 'Placed', company: 'Wipro', package: '9 LPA' }
  }
];
...
// BarChart component
const BarChart = ({ title, labels, datasets, stacked = false }) => {
  const options = {
    responsive: true,
    plugins: {
      legend: {
...
const Students = () => {
  ...
  const generateStudentPerformanceData = useCallback((filteredStudents) => {
    ...
    const avgGpaByYear = years.map((year) => {
      ...
      return yearStudents.reduce((sum, s) => sum + s.gpa, 0) / yearStudents.length;
    });
      ...
      datasets: [
        {
          ...
          data: avgGpaByYear,
          borderColor: view === 'placed' ? 'rgba(16, 185, 129, 1)' : 'rgba(20, 184, 166, 1)',
          backgroundColor: view === 'placed' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(20, 184, 166, 0.1)',
          fill: true,
        },
      ],
    });
  ...
  const generateStudentDistributionData = useCallback((filteredStudents) => {
    if (selectedDepartment === 'All' && selectedCourse === 'All') {
      const deptLabels = departments.map((d) => d.name);
      const deptCounts = deptLabels.map(
        (dept) => filteredStudents.filter((s) => s.department === dept).length
      );
        ...
        datasets: [
          {
            label: view === 'placed' ? 'Placed Students' : 'All Students',
            data: deptCounts,
            backgroundColor: view === 'placed' ? 'rgba(16, 185, 129, 0.8)' : 'rgba(20, 184, 166, 0.8)',
          },
        ],
      });
    }
      ...
      const deptCourses = courses
        .filter((c) => c.departmentId === departments.find((d) => d.name === selectedDepartment)?.id)
        .map((c) => getShortName(c.name));
      
      const courseCounts = deptCourses.map(
        (course) => filteredStudents.filter((s) => s.course === course).length
      );
  ...
  }, [view, selectedDepartment, selectedCourse, searchQuery, generateStudentPerformanceData, generateStudentDistributionData]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Students Table */}
      <motion.div
        className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 overflow-x-auto"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.3 }}
      >
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-4">
          <div className="flex items-center bg-gray-100 rounded-lg pl-4 pr-2 py-2 w-full md:w-auto">
            ...
            <input
              ...
              className="bg-transparent border-none outline-none ml-2 w-full text-gray-700 placeholder-gray-500"
            />
          </div>
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <select
              ...
              className="bg-white border border-gray-200 rounded-lg px-4 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500"
            >
            ...
            <div className="flex space-x-2">
              <button
                onClick={() => setView('all')}
                className={`px-4 py-2 rounded-md ${view === 'all' ? 'bg-teal-100 text-teal-600' : 'bg-gray-100 text-gray-600'}`}
              >
                All Students
              </button>
              <button
                onClick={() => setView('placed')}
                className={`px-4 py-2 rounded-md ${view === 'placed' ? 'bg-teal-100 text-teal-600' : 'bg-gray-100 text-gray-600'}`}
              >
                Placed Students
              </button>
            </div>
        ...
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-800">
            {view === 'placed' ? 'Placed Students' : 'All Students'} ({filteredStudents.length})
          </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            <h3 className="text-lg font-semibold text-gray-800 mb-4">
              {view === 'placed' ? 'Placed Students Performance' : 'Student Performance by Year'}
            </h3>
          ...
          >
            <h3 className="text-lg font-semibold text-gray-800 mb-4">
              {view === 'placed' ? 'Placed Students Distribution' : 'Student Distribution'}
            </h3>
