...
import { ClipboardListIcon, CalendarIcon, CheckCircleIcon, XCircleIcon, ClockIcon, BookOpenIcon, ArrowRightIcon, FilterIcon } from 'lucide-react';
...
const Assignments = ({ darkMode }) => {
  ...
  // Sample assignments data with department-specific assignments
  ...
  // Filter assignments based on department, status, and search query
  const filteredAssignments = allAssignments.filter(assignment => {
    ...
    const searchMatch = assignment.title.toLowerCase().includes(searchQuery.toLowerCase()) || assignment.course.toLowerCase().includes(searchQuery.toLowerCase()) || assignment.description.toLowerCase().includes(searchQuery.toLowerCase());
    return departmentMatch && statusMatch && searchMatch;
  });
  // Calculate statistics based on the filtered assignments
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <div className={`relative rounded-lg overflow-hidden backdrop-blur-sm ${darkMode ? 'bg-gray-700/50' : 'bg-gray-100/80'} border ${darkMode ? 'border-gray-600' : 'border-gray-200'}`}>
              <select value={status} onChange={e => setStatus(e.target.value)} className="appearance-none bg-transparent py-2 pl-3 pr-10 outline-none text-sm">
                {statuses.map(stat => <option key={stat} value={stat}>
                    {stat}
                  </option>)}
              </select>
              <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2">
        ...
        delay: 0.4
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-2xl font-bold mt-1">
                {lateAssignments + missedAssignments}
              </h3>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        delay: 0.1
          ...
          <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800 shadow-[5px_5px_10px_#1f2937,-5px_-5px_10px_#374151]' : 'bg-white shadow-[5px_5px_10px_#bebebe,-5px_-5px_10px_#ffffff]'}`}>
            ...
            {filteredAssignments.length === 0 ? <div className="text-center py-10">
                <p className="text-gray-500">
                  No assignments found matching your filters.
                </p>
              </div> : <motion.div variants={container} initial="hidden" animate="show" className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                {filteredAssignments.map(assignment => <motion.div key={assignment.id} variants={item} whileHover={{
              y: -5,
              transition: {
                duration: 0.2
              }
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                      ...
                      {assignment.grade && <div className={`mt-2 md:mt-0 px-3 py-1 rounded-lg text-sm ${darkMode ? 'bg-green-900/20 text-green-300' : 'bg-green-50 text-green-800'}`}>
